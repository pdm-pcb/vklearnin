...
// =============================================================================
void Renderer::submit_draw(GeneratedMesh const &mesh, Mat4 const &model_matrix)
{
...
void Renderer::record_commands() {
    ...
    auto &color_buffer    = _color_buffers[_frame_index];
...
void Renderer::set_textures(std::vector<Texture2D> const &textures) {
    ...
    for(auto const &texture : textures) {
        ...
        _texture_sets.back()
            ...
            .add_combined_sampler(texture.image())
            .write_set();
    }
...
// =============================================================================
void Renderer::_init_depth_buffers() {
    if(!_depth_buffers.empty()) {
        CONSOLE_CRITICAL("Cannot re-init depth buffers.");
        return;
    }

    _depth_buffers.resize(Swapchain::images().size());

    for(auto &depth_buffer : _depth_buffers) {
        depth_buffer.create(
            Swapchain::render_area(),
            PhysicalDevice::depth_format(),
...
// =============================================================================
void Renderer::_init_color_buffers() {
    if(!_color_buffers.empty()) {
        CONSOLE_CRITICAL("Cannot re-init depth buffers.");
        return;
    }

    _color_buffers.resize(Swapchain::images().size());
...
//     auto const &swapchain_extent = Swapchain::render_area().extent;
...
//         framebuffer.create_shadow_map(shadow_map_render_area);
//     }
    ...
    _flat_color_pipeline
        ...
        .create(
            Pipeline::Config {
                ...
                .viewport_extent = Swapchain::render_area().extent,
                .viewport_offset = Swapchain::render_area().offset,
