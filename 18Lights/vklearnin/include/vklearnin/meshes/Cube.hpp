...
public:
    ...
    requires std::is_same_v<VertexType, VertexMaterial>
    {
        Mesh<VertexType>::_set_vertices({
            ...
            {{ -scale,  scale,  scale, 1.0f },  Vec4::unit_z, { 0.0f, 1.0f }}, // 1
            {{  scale,  scale,  scale, 1.0f },  Vec4::unit_z, { 1.0f, 1.0f }}, // 2
            {{  scale, -scale,  scale, 1.0f },  Vec4::unit_z, { 1.0f, 0.0f }}, // 3

            // back face
            {{  scale, -scale, -scale, 1.0f }, -Vec4::unit_z, { 1.0f, 0.0f }}, // 4
            {{  scale,  scale, -scale, 1.0f }, -Vec4::unit_z, { 1.0f, 1.0f }}, // 5
            {{ -scale,  scale, -scale, 1.0f }, -Vec4::unit_z, { 0.0f, 1.0f }}, // 6
            {{ -scale, -scale, -scale, 1.0f }, -Vec4::unit_z, { 0.0f, 0.0f }}, // 7

            // top face
            {{ -scale,  scale,  scale, 1.0f },  Vec4::unit_y, { 0.0f, 0.0f }}, // 8
            {{ -scale,  scale, -scale, 1.0f },  Vec4::unit_y, { 0.0f, 1.0f }}, // 9
            {{  scale,  scale, -scale, 1.0f },  Vec4::unit_y, { 1.0f, 1.0f }}, // 10
            {{  scale,  scale,  scale, 1.0f },  Vec4::unit_y, { 1.0f, 0.0f }}, // 11

            // bottom face
            {{ -scale, -scale, -scale, 1.0f }, -Vec4::unit_y, { 1.0f, 0.0f }}, // 12
            {{ -scale, -scale,  scale, 1.0f }, -Vec4::unit_y, { 1.0f, 1.0f }}, // 13
            {{  scale, -scale,  scale, 1.0f }, -Vec4::unit_y, { 0.0f, 1.0f }}, // 14
            {{  scale, -scale, -scale, 1.0f }, -Vec4::unit_y, { 0.0f, 0.0f }}, // 15

            // left face
            {{ -scale, -scale, -scale, 1.0f }, -Vec4::unit_x, { 0.0f, 0.0f }}, // 16
            {{ -scale,  scale, -scale, 1.0f }, -Vec4::unit_x, { 0.0f, 1.0f }}, // 17
            {{ -scale,  scale,  scale, 1.0f }, -Vec4::unit_x, { 1.0f, 1.0f }}, // 18
            {{ -scale, -scale,  scale, 1.0f }, -Vec4::unit_x, { 1.0f, 0.0f }}, // 19

            // right face
            {{  scale, -scale,  scale, 1.0f },  Vec4::unit_x, { 1.0f, 0.0f }}, // 20
            {{  scale,  scale,  scale, 1.0f },  Vec4::unit_x, { 1.0f, 1.0f }}, // 21
            {{  scale,  scale, -scale, 1.0f },  Vec4::unit_x, { 0.0f, 1.0f }}, // 22
