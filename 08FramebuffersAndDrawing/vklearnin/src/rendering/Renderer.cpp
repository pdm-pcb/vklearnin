...
void Renderer::_present() {
    auto &present_queue = _logical_device.present_queue();
    auto &cmd_buffer    = present_queue.cmd_buffer(_target_image_index);
    ...
    auto result = present_queue.native(_target_image_index).submit(
        submit_info,
        _present_fences[_current_framebuffer]
    );
    if(result != vk::Result::eSuccess) {
    ...
    // along the way
    result = present_queue.native(_target_image_index).presentKHR(present_info);
    ...
    _pipeline            { pipeline }
{
    ...
    for(uint32_t image_idx = 0;
        ...
        ++image_idx)
    {
        _framebuffers.emplace_back(logical_device.native(), swapchain, pipeline);
    }
}
