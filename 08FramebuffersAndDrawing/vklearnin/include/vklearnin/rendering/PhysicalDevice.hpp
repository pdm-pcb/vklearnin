...
public:
    ...
    static void set_current(PhysicalDevice *device) { _current = device; }

    void init(const vk::Instance &graphics_instance);
    void select_device(const vk::SurfaceKHR &surface);

    inline auto graphics_queue_index()   const { return _graphics_queue_index; }
    inline auto present_queue_index()    const { return _present_queue_index;  }
    inline const auto & family_indices() const { return _family_indices;  }
    inline const auto & native()         const { return _physical_device; }
...
private:
    ...
    static PhysicalDevice *_current;
