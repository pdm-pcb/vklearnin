...
Vulkan has no global state, so each application needs to track its own. The instance will store this information, and creating one initializes the Vulkan library. First, let's look at the member variables for the class.
...
The following members are intended to populate a [`vk::InstanceCreateInfo`](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkInstanceCreateInfo.html) structure.
...
### Create target window and surface
