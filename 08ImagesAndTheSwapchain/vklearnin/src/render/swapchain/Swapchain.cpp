...
void Swapchain::destroy() {
    for(auto &sync : _image_sync) {
        CONSOLE_TRACE(
            ...
            reinterpret_cast<uint64_t>(VkFence(sync.present_fence))
        );
        LogicalDevice::native().destroySemaphore(sync.available);
        LogicalDevice::native().destroySemaphore(sync.draw_complete);
        LogicalDevice::native().destroyFence(sync.present_fence);
    }
...
void Swapchain::_create_sync() {
    ...
    for(auto &sync : _image_sync) {
        ...
        std::tie(result, sync.available) =
            LogicalDevice::native().createSemaphore(sem_info);
        if(result != vk::Result::eSuccess) {
            CONSOLE_CRITICAL(
                "Unable to create available semaphore: '{}'",
                to_string(result)
        );
            return;
        }
        ...
        std::tie(result, sync.present_fence) =
            LogicalDevice::native().createFence(fence_info);
        if(result != vk::Result::eSuccess) {
            CONSOLE_CRITICAL(
                "Unable to create swapchain present fence: '{}'",
                to_string(result)
        );
            return;
        }
